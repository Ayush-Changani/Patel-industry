...
const Item_Type_list = () => {
  ...
  const loadItemTypes = async () => {
    try {
      ...
      Swal.fire("Error", "Failed to fetch item types", "error");
      setOriginalData([]);
      setFilteredData([]);
      setData([]);
    }
  ...
  const handleSearch = (text) => {
    ...
    const filtered = originalData.filter((row) =>
      row.item_type?.toLowerCase().includes(lower)
    );
  ...
  /* ================= ACTIONS ================= */
  const handleAdd = () => navigate("/Item_Type_Master");

  const handleEdit = (row) =>
    navigate(`/Item_Type_Master?eid=${row.it_id}`);

  const handleView = (row) =>
    navigate(`/Item_Type_Master?view_id=${row.it_id}`);

  const handleDelete = async (row) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `Delete item type: ${row.item_type}?`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const res = await api.post(
          `/i_pi_item_type_mst_delete?it_id=${row.it_id}`
        );

        if (res.data?.Status === 1 || res.data?.status === 1) {
          Swal.fire(
            "Deleted",
            res.data?.message || "Item Type deleted successfully",
            "success"
          );
          loadItemTypes();
        } else {
