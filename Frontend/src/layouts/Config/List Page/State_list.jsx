...
const State_list = () => {
  ...
  const handleSearch = (text) => {
    ...
    const filtered = originalData.filter(
      (row) =>
        ...
        row.gst_code?.toLowerCase().includes(lower)
    );
  ...
  /* ================= ACTIONS ================= */
  const handleAdd = () => navigate("/State_master");

  const handleEdit = (row) =>
    navigate(`/State_master?eid=${row.st_id}`);

  const handleView = (row) =>
    navigate(`/State_master?view_id=${row.st_id}`);

  const handleDelete = async (row) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `You want to delete ${row.state_name}?`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const res = await api.post(
          `/i_pi_state_mst_delete?st_id=${row.st_id}`
        );

        if (res.data?.Status === 1 || res.data?.status === 1) {
          Swal.fire(
            "Deleted",
            res.data?.message || "State has been deleted.",
            "success"
          );
          loadStates();
        } else {
